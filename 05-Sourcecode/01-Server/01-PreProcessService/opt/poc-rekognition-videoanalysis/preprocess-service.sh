...
do
  	...
  	if [ "$QUEUE_MESSAGE" != "null" ] ; then
        ...
        aws lambda invoke --invocation-type Event --function-name RVA_IoT_publish_message_function --region $EC2_REGION --payload '{{"topic": $IOT_TOPIC, "type": "status", "payload": {"message": "Extracting frames from video", "percentage": 40}}}'
        ...
        aws lambda invoke --invocation-type Event --function-name RVA_IoT_publish_message_function --region $EC2_REGION --payload '{{"topic": $IOT_TOPIC, "type": "status", "payload": {"message": "Analyzing frames", "percentage": 60}}}'
